...
%% Leggere punti di discretizzazione da XFOIL (es. NACA 0012)
% Apriamo XFOIL da Ubuntu con comando "xfoil" in minuscolo; selezioniamo il
% Naca che vogliamo (es. NACA 0012); andiamo su PPAR e scriviamo N per
% cambiare il numero di nodi--> scegliamo 103 nodi (cioè 102 pannelli).

% Premiamo SAVE e così salviamo i punti di discretizzazione su un file di
% testo. Copio il file di testo nella cartella matlab corrente. 

% Sistemo le righe e colonne che voglio estrarre dal file di testo (occhio
% a quando c'è la notazione scientifica (es. E-01), perchè potrebbe andare
% a finire nella colonna successiva--> devo spostare il margine di tale
% colonna). Vado su "output type" e seleziono "numeric matrix".

% Tasto destro del mouse sul file e poi "import data". Vado su freccetta di
% "import selection" e premo "generate fuction".

% Salvo la funzione senza toccare niente e la uso qui sotto, chiamando tra
% 2 apostrofi il nome del file di testo da importare.

naca0012nodes=importfile('naca0012nodes.txt');
...
%% Calcolo Cp nei vari punti di controllo (è totalmente sballato!)
Cp=zeros(N_pann,1);
for i=1:N_pann
