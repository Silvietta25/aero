...
% velocità asintotica
U_inf = [51 0 0]';
z=3000; % quota di volo in metri
[p_amb,T_amb]=atm_standard(z);
gamma_a=1.4;
R=287;
% densità
rho=p_amb/(R*T_amb);
...
% angolo_di_rollio = [0];
angolo_di_incidenza_aero = [7.42 -1.55]'; % alpha
...
L_tot=L_3D{1,1}+L_3D{1,2};
CL_tot=L_tot/(0.5*rho*(norm(U_inf)^2*superficie_alare(1)));
D_tot=D_3D{1,1}+D_3D{1,2};
CD_tot=D_tot/(0.5*rho*(norm(U_inf)^2*superficie_alare(1)));

%% Momento di beccheggio (positivo a cabrare)
CA_ala=estremi_ala{1,1}(:,1)+(estremi_ala{1,1}(:,2)-estremi_ala{1,1}(:,1))/4;
M_CG_ala=-L_3D{1,1}*(CA_ala(1));
CA_coda=estremi_ala{1,2}(:,1)+(estremi_ala{1,2}(:,2)-estremi_ala{1,2}(:,1))/4;
M_CG_coda=-L_3D{1,2}*((CA_coda(1)));
M_CG_tot=M_CG_ala+M_CG_coda;
% Adimensionalizziamo con grandezze ala
c_media1=corda_radice(1)*(1+rastremazione(1))/2;
Cm_CG_tot=M_CG_tot/(0.5*rho*(norm(U_inf)^2)*superficie_alare(1)*c_media1);

%% Calcolo efficienza e consumo dovuto a drag indotto
E=CL_tot/CD_tot;
% La resistenza risulta molto bassa rispetto a quella reale, perchè non ho
% spessore e ho ipotesi di effetti viscosi trascurabili
Psi=1-1/(Endurance_target*BSFC/(2*eta_p*E*sqrt(CL_tot))*sqrt(2*W/(rho*c_media1*apertura_alare(1)))+1)^2;
W_fuel=Psi*W; % peso combustibile consumato in Newton
W_2=W-W_fuel; % peso finale velivolo in [N]

%% Autonomia chilometrica (range)
Range=eta_p*E/BSFC*log(1/(1-Psi));
Range_km=Range/1000;

%% Velocità finale crociera
U_2=sqrt(W_2/(0.5*rho*superficie_alare(1)*CL_tot));

%% Stabilità longitudinale (cambiare dati anche qui se necessario)
U_inf = [51 0 0]';
rho=0.9092;
config.NCorpi = 2;
config.RootChord = [1.473 1.0311]; 
config.DihedralAngle = [0 0]; 
config.SweepAngle = [0 0]; 
config.TaperRatio = [1 0.8]; 
config.Span = [11 0.25*11]; 
config.LEPosition_X = [0 6]; 
config.LEPosition_Y = [0 0]; 
config.LEPosition_Z = [0.5 0]; 
config.RotationAngle_X = [0 0]; 
config.RotationAngle_Z = [0 0]; 
config.SemiSpanwiseDiscr = [30 8];
config.ChordwiseDiscr = [10 5];

angolo_di_incidenza_aero = [7.42 -1.55]';
Perturbaz_Alpha_velivolo=1;
...
alpha_vect=-3:0.4:3;
Cm_CG_vect=[];
for h=1:length(alpha_vect)
    Perturbaz_Alpha_velivolo=alpha_vect(h);
    [M_CG_tot,Cm_CG_tot]=momentobeccheggio(config,...
    angolo_di_incidenza_aero(1),angolo_di_incidenza_aero(2),Perturbaz_Alpha_velivolo,U_inf,rho);
    Cm_CG_vect(end+1)=Cm_CG_tot;
end
figure
plot(alpha_vect,Cm_CG_vect,'Linewidth',2,'Color','r')
grid on