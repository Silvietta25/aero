...
delta_alpha=4;
alpha2 = alpha1+delta_alpha.*pi/180;
...
Gap_x=0.05;
Gap_y=0;
x12=x_nodes1(1)+Gap_x;
y12=y_nodes1(1)+Gap_y;
...
% Scegliamo una velocità del vento e una della barca
v_barca=[0 8];
U_vento=-4.5/sqrt(2)*[1 1]; % inclinato a -45° rispetto alla verticale
U_incid=-v_barca+U_vento;
% Trovo angolo tra asse longitudinale barca e velocità incidente e quello
% tra direzione vela e asse longitudinale
beta=acosd(dot(-U_incid,[0 1])/norm(U_incid));
theta=beta-alpha1;
% Assumo un'altezza della vela in metri
h=6;
% Calcolo portanza del tandem, attribuendola al profilo anteriore
L=0.5*rho*norm(U_incid)^2*h*Chord(1)*Cl1;
% Calcolo spinta e sideforce
Spinta=L*sind(beta);
Sideforce=-L*cos(beta); % negativa perchè verso sinistra
