...
% angolo_di_rollio = [0];
angolo_di_incidenza_trim = [7.42 -1.55]'; % alpha
...
alpha_pert = -3:0.4:3; % % alpha di perturbazione

% Inizializzazione vettori momento beccheggio
M_CG_tot_vect = zeros(1,length(alpha_pert));
Cm_CG_tot_vect = zeros(1,length(alpha_pert));

for a = 1:(length(alpha_pert) + 1)
    if a ~= length(alpha_pert) + 1
        angolo_di_incidenza_aero = angolo_di_incidenza_trim + alpha_pert(a);
    else
        angolo_di_incidenza_aero = angolo_di_incidenza_trim;
    end
    ...
    for i = 1:N_corpi
        CA{1,i} = estremi_ala{1,i}(:,1)+(estremi_ala{1,i}(:,2)-estremi_ala{1,i}(:,1))/4;
        M_CG{1,i} = -L_3D{1,i}*CA{1,i}(1);
    end
    M_CG_tot = sum(cell2mat(M_CG(1,:)));

    if a ~= length(alpha_pert) + 1
        M_CG_tot_vect(a) = M_CG_tot;
    end

    % Adimensionalizziamo con grandezze ala (superficie aerodinamica 1)
    c_media_1 = corda_radice(1)*(1+rastremazione(1))/2;
    Cm_CG_tot = M_CG_tot/(0.5*rho*(norm(U_inf)^2)*superficie_alare(1)*c_media_1);

    if a ~= length(alpha_pert) + 1
        Cm_CG_tot_vect(a) = Cm_CG_tot;
    end
...
Psi=1-1/(Endurance_target*BSFC/(2*eta_p*E*sqrt(CL_tot))*sqrt(2*W/(rho*c_media_1*apertura_alare(1)))+1)^2;
